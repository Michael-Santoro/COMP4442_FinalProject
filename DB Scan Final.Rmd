...
#transformed_images <- sapply(images$image[1:200],imageTransform) #applying function to first 200 images
#transformed_images <- sapply(images$image,imageTrasform) #applying function to total dataset

#Load in the saved .rdata file

load("transformed_images.RData") #loading presaved data that we computed 
...
transposed_tranformed_images <- t(transformed_images) #transposed dataframe to swap rows and columns for processing
...
db_cluster<- fviz_cluster(res, data=transposed_tranformed_images, geom = "point" ) #cluster visualization
db_cluster
```
...
#row <- transposed_tranformed_images["0a30baec-1938-47c5-b495-68a51701b43d",] #noise point 
row <- transposed_tranformed_images["f1c78283-2da0-45fc-8f20-4f874519d415",] #ideal
...
#Breakdown of Noise points by Label 
noise %>%
distinct(image, label) %>%
group_by(label) %>%
summarize("agent in teams" = n())
```
```{r}

#Plots showing label counts

sub.group <- images[]

sub.group["clusters"] <- res$cluster 

ggplot(images, aes(x = label, color=label)) +        # Create barchart of counts with ggplot2
  geom_bar() +
  ggtitle("Bar Plot of Label Counts From Cleaned images Dataframe (n > 300)") + 
  geom_text(stat='count', aes(label=..count..), vjust=-1, show.legend = FALSE) + 
...
```
```{r}
classification_distr <- ggplot(sub.group[which(sub.group$clusters==1),], aes(x = label, color=label)) +        # Create barchart of counts with ggplot2
  geom_bar() +
  ggtitle("Bar Plot of Label Counts From Cluster 1") + 
  geom_text(stat='count', aes(label=..count..), vjust=-1, show.legend = FALSE) + 
  ylim(c(0,1100))  #ylim based on max value from images table (1:x)

classification_distr
