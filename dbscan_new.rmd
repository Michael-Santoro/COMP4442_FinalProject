...
images <- images[which(images$label != "Not Sure"),]
```
...
```{r}
#Currently only first 100
transfromed_images <- sapply(images$image[1:100],imageTrasform)
```

```{r}
X <- array(transfromed_images,dim = c(400,400,100))
```
...
```{r}
res <- dbscan(X, eps = 0.1, minPts = 200)
res
```
